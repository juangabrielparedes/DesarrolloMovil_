    ...
    suspend fun getRequestsForOwner(ownerUid: String): List<Request> {
        return try {
            ...
            for (chunk in bizIds.chunked(10)) {
                android.util.Log.d("DEBUG", "Buscando requests para businessIds: $chunk")
                ...
                android.util.Log.d("DEBUG", "Solicitudes encontradas: ${snap.documents.size}")
            ...
            android.util.Log.d("DEBUG", "Total solicitudes: ${result.size}")
            result
        } catch (e: Exception) {
